...
const formatters: Record<Format, FormatterFn> = {
    ...
    title_url_colon: (tabs) => tabs.map((t) => `${normalizeTitle(t.title)}: ${t.url}`).join('\n'),
    title_url_newline: (tabs) =>
        tabs.map((t) => `${normalizeTitle(t.title)}\n${t.url}`).join('\n\n'),
    markdown: (tabs) => tabs.map((t) => `- [${normalizeTitle(t.title)}](${t.url})`).join('\n'),
    csv: (tabs) => {
        if (tabs.length === 0) return '';
        const header = '"title","url"';
        const rows = tabs.map((t) => `${escapeCsv(normalizeTitle(t.title))},${escapeCsv(t.url)}`);
